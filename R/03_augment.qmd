...
quality_table <- genome_metadata_clean |>
  dplyr::select(Genome, Completeness, Contamination) |>
  dplyr::mutate(
    Completeness_quality = dplyr::case_when(
      Completeness > 90                          ~ "High",
      Completeness >= 50 & Completeness <= 90    ~ "Medium",
      Completeness < 50                          ~ "Low",
      TRUE                                       ~ NA_character_
    ),
    Contamination_quality = dplyr::case_when(
      Contamination < 5    ~ "High",
      Contamination < 10   ~ "Medium",
      Contamination >= 10  ~ "Low",
      TRUE                 ~ NA_character_
    ),
    Overall_quality = dplyr::case_when(
...
genome_metadata_aug <- genome_metadata_clean |>
  dplyr::left_join(quality_table, by = "Genome")
```
