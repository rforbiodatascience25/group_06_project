...
phylum_prop_plot <-
  genome_metadata_aug |>
  dplyr::count(Phylum, 
               endometriosis_associated) |>
  dplyr::group_by(Phylum) |>
  dplyr::mutate(prop = n / sum(n)) |>
  ggplot2::ggplot(ggplot2::aes(x    = Phylum,
                               y    = prop,
...
gc_violin_plot <-
  genome_metadata_aug |>
  ggplot2::ggplot(ggplot2::aes(x    = endometriosis_associated,
                               y    = GC_content,
                               fill = endometriosis_associated
  )) +
  ...
  ggplot2::geom_boxplot(width = 0.18,
                        outlier.size = 0.7,
                        alpha = 0.9,
                        colour = "black"
  ) +
...
genome_metadata_aug |>
  dplyr::select(endometriosis_associated,
                Completeness,
                Contamination
  ) |>
  tidyr::pivot_longer(
